    ...
    methods
       ...
       % TODELETE     obj.initialState.ghost  = ghost_init;
        ...
        function nextState = f(obj, in)
          ...
          % TODELETE              thePacmanWallsV(obj.presentState.ghost(2)+1,obj.presentState.ghost(1):obj.presentState.ghost(1)+1) = 1;
             ...
             if(in(5) == 1) % Pacman get Up
               ...
               % TODELETE         thePacmanWallsH(obj.presentState.ghost(2):obj.presentState.ghost(2)+1,obj.presentState.ghost(1)+1) = 1;
...
%        % TODELETE                    (obj.sameY_position*obj.wallsVBetweenOne(obj.presentState.pacman, obj.presentState.ghost)) + ...
...
%                            (obj.sameX_position*obj.wallsHBetweenOne(obj.presentState.pacman, obj.presentState.ghost));
        ...
        %       pacman position (X,Y)
        %  % TODELETE     Ghost  position (X,Y)
        %       WallsV matrix
        %       WallsH matrix
        %   % TODELETE    Counter of caught
        %       Boolean of escape
        %       Vector with wallsAroundPacman
        %  % TODELETE     Vector with wallsAroundGhost
        %  % TODELETE     Vector with ghostSeesPacman
        %
        function  out = g(obj)
            % walls Around pacman
...
%  % TODELETE           Wdown_ghost = [obj.presentState.wallsH(obj.presentState.ghost(2):size(obj.presentState.wallsH,1), obj.presentState.ghost(1)); 1];
...
%    % TODELETE                             obj.sameX_position*obj.wallsHBetween(obj.presentState.ghost,obj.presentState.pacman), ...
...
%         end
%         
%     % TODELETE    %% Function that return 1 if ghots and pacman are on the same Y line
